...
| 3c | 按 plan `RUN=1` 跑框架 F1 | diamond → `F2_eggnog.sh`（=F1 内步）→ IPS |
| 4 | [**验收勾选表**](验收勾选表.md) | 认清 **F-L1** 档位（≠今晚必交满跑） |

**步 3c 说明：** 编号对照见 [FAQ](FAQ_入门.md)。上集群前必勾 [Done when](https://github.com/Xuzhen-Li/gene-function-annotation/wiki/Start-tonight#done-when) + `python3 pipeline/print_qc_commands.py` 绿（无 `[STOP]`）；未勾不许 `RUN=1`。
...
**抗病 / 葡萄最短补丁：** 答案直接抄 [`examples/plant_sim/01_vitis_s1_s7/`](../../examples/plant_sim/01_vitis_s1_s7/)（`want_nlr: true` → F1+F8）；口播红线见 [Wiki Red-lines F8](https://github.com/Xuzhen-Li/gene-function-annotation/wiki/Red-lines#f8-screen)。
